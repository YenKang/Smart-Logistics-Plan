...
\end{minipage}
\normalsize
\caption{\large First two big United Parcel Service of America Inc.'s revenue.}
...
The first table named user-table keeps the account information of registered users. Table 3.1 shows the data format of user-table. As mentioned in Chapter 3.2.1, the attribute \textbf{device\_key} is the token generated by the application. The token is assigned to the mobile device rather than the user, and it will be updated when the user logins on different devices. Therefore, the SUMO server can know which device is bind to the user by the unique token. The attribute username is applied to identify the user who places the order.
...
If the user is a \textbf{receiver} in an order, he or she can choose the pick-up time of the cargo according to the \textbf{status of logistic activity}, which means the shipping situation of the cargo. Then, the \textbf{pick-up-time} page would pop up, which is the same as the page in Figure 4.5. 
...
Besides, a system administrator could examine the history orders without logging in. Figure 4.8 shows the searching page of the order. The order can be looked up by entering the unique order number. 
