					...
					if (clientInfos.size() > 0) {
						for (int j =0; j < clientInfos.size(); j++) {
							...
							if (clientInfo.getRequestNo() == 0) {
								...
								JDBC_AVD deviceID_getter = new JDBC_AVD();
								String sender_DID = deviceID_getter.QueryDeviceKey(sender_name);
								String receiver_DID = deviceID_getter.QueryDeviceKey(receiver_name);
